...
public class ItemSpongeItemHandler implements IItemHandlerModifiable, INBTSerializable<NBTTagCompound>
{
    ...
    {
        if (nbt.hasKey(NbtConsts.ITEMS_KEY) && nbt.hasKey(NbtConsts.REALSTACKSIZE_KEY, Constants.NBT.TAG_INT))
        {
            setAbsorbedStack(new ItemStack(nbt.getTagList(NbtConsts.ITEMS_KEY, Constants.NBT.TAG_COMPOUND).getCompoundTagAt(0)));
            realStackSize = nbt.getInteger(NbtConsts.REALSTACKSIZE_KEY);
        }
    }
