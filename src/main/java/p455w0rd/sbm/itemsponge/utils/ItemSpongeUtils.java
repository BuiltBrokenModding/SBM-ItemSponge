package p455w0rd.sbm.itemsponge.utils;
...
import p455w0rd.sbm.itemsponge.init.ModGlobals.NBT;
import p455w0rd.sbm.itemsponge.init.ModItems;
import p455w0rd.sbm.itemsponge.init.ModNetworking;
...
public class ItemSpongeUtils {
				...
				for (int i = 0; i < inv.getSizeInventory(); i++) {
					...
					if (isItemValid(sponge.getValue(), currentStack) && sponge.getValue().hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null)) {
						...
						if (itemHandler != null && itemHandler.isItemValid(0, currentStack)) {
							...
							sponge.getValue().getTagCompound().setTag(NBT.INVENTORY_KEY, itemHandler.serializeNBT());
							ModNetworking.sendStackSyncPacketToClient((EntityPlayerMP) player, sponge.getKey(), sponge.getValue().serializeNBT());
						}
					}
				}
			}
	...
	*/
	public static boolean absorbEntityItems(TileItemSponge tile) {
	 ...
	 */
	private static List<EntityItem> getAbsorbableEntityItems(TileItemSponge tile) {
	...
	private static List<EntityItem> getAbsorbableEntityItems(World world, BlockPos vec, ItemStack comparableStack) {
		...
		for (EntityItem item : world.getEntitiesWithinAABB(EntityItem.class, new AxisAlignedBB(vec.getX() - radius, vec.getY() - radius, vec.getZ() - radius, vec.getX() + radius, vec.getY() + radius, vec.getZ() + radius))) {
			...
			if (nbt.hasKey(NBT.DEMAGNETIZE_KEY, Constants.NBT.TAG_BYTE) && nbt.getBoolean(NBT.DEMAGNETIZE_KEY)) {
				continue;
			}
	...
	/**
	 * Checks whether or not the {@link TileItemSponge} has a redstoner signal
	 */
	public static boolean hasRedstoneSignal(TileItemSponge tile) {
	...
	/**
	 * Sets {@link net.minecraft.tileentity.TileEntity TileEntity} NBT when the {@link net.minecraft.item.Item Item} version of the Item Sponge ({@link p455w0rd.sbm.itemsponge.items.ItemBlockItemSponge ItemBlockItemSponge.class})
	 * is placed into the world as a {@link net.minecraft.Block Block}
	 */
	...
	public static ItemStack setAbsorbedItem(@Nonnull ItemStack sponge, ItemStack absorbedItem) {
		if (sponge.isEmpty() || sponge.getItem() != ModItems.ITEM_SPONGE_ITEMBLOCK) {
		...
		if (absorbedItem.isEmpty()) {
			sponge = new ItemStack(ModItems.ITEM_SPONGE_ITEMBLOCK);
		}
		else {
	...
	public static ItemStack getAbsorbedItem(ItemStack sponge) {
		if (!sponge.isEmpty() && sponge.getItem() == ModItems.ITEM_SPONGE_ITEMBLOCK && sponge.hasTagCompound() && sponge.getTagCompound().hasKey(NBT.INVENTORY_KEY, Constants.NBT.TAG_COMPOUND)) {
			return getCachedStack(sponge.getTagCompound().getCompoundTag(NBT.INVENTORY_KEY).getTagList(NBT.ITEMS_KEY, Constants.NBT.TAG_COMPOUND).getCompoundTagAt(0));
		}
		return ItemStack.EMPTY;
	}
	...
	public static NBTTagCompound getSerializedSpongeStack(@Nonnull ItemStack sponge, @Nonnull ItemStack absorbedStack, int stackSize) {
		...
		sponge.getTagCompound().setTag(NBT.INVENTORY_KEY, createSerializedItemHandlerList(absorbedStack, stackSize));
	...
	public static NBTTagCompound getSerializedItemHandlerFromSponge(@Nonnull ItemStack sponge) {
		if (sponge.hasTagCompound() && sponge.getTagCompound().hasKey(NBT.INVENTORY_KEY, Constants.NBT.TAG_COMPOUND)) {
			return sponge.getTagCompound().getCompoundTag(NBT.INVENTORY_KEY);
		}
		return null;
	}
	...
	public static NBTTagCompound createSerializedItemHandlerList(@Nonnull ItemStack absorbedStack, int stackSize) {
		...
		absorbedItemTag.setInteger(NBT.SLOT_KEY, 0);
		absorbedStack.setCount(1);
		absorbedStack.writeToNBT(absorbedItemTag);
		nbtTagList.appendTag(absorbedItemTag);
		NBTTagCompound invNBT = new NBTTagCompound();
		invNBT.setTag(NBT.ITEMS_KEY, nbtTagList);
		invNBT.setInteger(NBT.SIZE_KEY, 1);
		invNBT.setInteger(NBT.REALSTACKSIZE_KEY, stackSize);
		return invNBT;
	}
	 ...
	 */
	private static int getAbsorbedItemCount(TileItemSponge tile) {
	...
	private static int getAbsorbedItemCount(NBTTagCompound nbt) {
		if (nbt.hasKey(NBT.INVENTORY_KEY, Constants.NBT.TAG_COMPOUND)) {
			NBTTagCompound invNBT = nbt.getCompoundTag(NBT.INVENTORY_KEY);
			if (invNBT.hasKey(NBT.REALSTACKSIZE_KEY, Constants.NBT.TAG_INT)) {
				return invNBT.getInteger(NBT.REALSTACKSIZE_KEY);
			}
		}
		return 0;
	}
	...
	public static boolean isSponge(ItemStack stack) {
		return !stack.isEmpty() && stack.getItem() == ModItems.ITEM_SPONGE_ITEMBLOCK;
	}
